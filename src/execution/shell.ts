...
export async function executeCommand(
   command: string,
   options?: { truncateOutput?: number; cwd?: string; env?: Record<string, string> }
  ...
  try {
    ...
    // We need to use dynamic template literal evaluation
     const result = await executeShellCommand(command, options?.cwd, options?.env)
   ...
   try {
     ...
     // The quiet() method suppresses output and returns Buffers
      ...
      if (cwd) shell.cwd(cwd)
       if (env) shell.env({ ...process.env, ...env })
